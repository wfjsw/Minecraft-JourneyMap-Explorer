// modified from https://github.com/unbam/Leaflet.SlideMenu
...
L.Control.SlideMenu = L.Control.extend({
    ...
    onAdd: function (map) {
        ...
        btn.innerHTML = this.options.hint
        btn.style.paddingLeft = btn.style.paddingRight = '8px'
        btn.style.paddingTop = btn.style.paddingBottom = '5px'
        link.appendChild(btn)

        this._menu = L.DomUtil.create('div', 'leaflet-menu', map._container);

        this._menu.style.width = this.options.width;
        this._menu.style.maxWidth = this.options.maxwidth;
        this._menu.style.height = this.options.height;

        if (this._isHorizontal) {
            this._frominit = -(parseInt(this.options.width, 10));
            ...
            this._frominit = -(parseInt(this.options.height, 10));
            if (this._isLeftPosition) {
        ...
        if (this._isHorizontal) {
            this._ispx = this.options.width.slice(-1) == 'x' ? true : false;
            this._unit = parseInt(this.options.width, 10) * parseInt(this.options.changeperc, 10) / 100;
        }
        else {
            this._ispx = this.options.height.slice(-1) == 'x' ? true : false;
            this._unit = parseInt(this.options.height, 10) * parseInt(this.options.changeperc, 10) / 100;
        }
    ...
    open: function () {
        this._isOpen = true
        this._animate(this._menu, this._frominit, 0, true, this._ispx, this._unit);
    },

    close: function () {
        this._isOpen = false
        this._animate(this._menu, 0, this._frominit, false, this._ispx, this._unit);
    },
